...
func TestMain(m *testing.M) {
	...
	suite.
		...
		Setup(deployEchoes).
		Setup(verifyInjectedFirewallBackend).
		Setup(deployExtProc).
		Run()
}

func verifyInjectedFirewallBackend(ctx resource.Context) error {
	if ambientMode || deployAsSandbox {
		return nil
	}

	pods, err := ctx.Clusters().Default().Kube().CoreV1().Pods(ns.Name()).List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("list echo pods to verify injected firewall backend: %w", err)
	}
	...
	for _, pod := range pods.Items {
		...
		for _, container := range containers {
			...
			for _, envVar := range container.Env {
				if envVar.Name != "FIREWALL_BACKEND" {
					continue
				}
				if envVar.Value != firewallBackend {
					return fmt.Errorf(
						"pod %s has injected FIREWALL_BACKEND=%q, want %q",
						pod.Name, envVar.Value, firewallBackend,
					)
				}
				verified++
			}
	...
	if verified == 0 {
		return fmt.Errorf("no injected istio-proxy container with FIREWALL_BACKEND found in namespace %s", ns.Name())
	}
	return nil
}
