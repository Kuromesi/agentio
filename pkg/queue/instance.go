...
// NewWithSync instantiates a named queue with a sync callback.
...
func (q *queueImpl) processNextItem() bool {
	...
	if err := task.task(); err != nil {
		delay := q.delay
		log.Warn("work item failed; retrying after delay", "queue", q.id, "delay", delay, "error", err)
...
func (q *queueImpl) Run(stop <-chan struct{}) {
	log.Debug("queue started", "queue", q.id)
	defer func() {
		q.closeOnce.Do(func() {
			log.Debug("queue closed", "queue", q.id)
			close(q.closed)
		})
	}()
	go func() {
