...
// WithoutTransformInstrumentation disables the per-transform timing, recompute
// counting, and slow-transform warning for one collection. Instrumentation is
// on by default; opt out only where the transform is so cheap that the
// measurement would dominate it.
func WithoutTransformInstrumentation() CollectionOption {
	return func(c *collectionOptions) {
		c.instrumentation = false
	}
}
