...
func (j *mergejoin[T]) onSubCollectionEventHandler(o []Event[T]) {
	...
	for _, ev := range items {
		...
		if ev.Event == controllers.EventDelete {
			...
			if !f {
				j.log.Error("invalid event: deletion of non-existent object", "input_key", objKey)
				continue
			}
			e := Event[T]{
			...
			if j.log.DebugEnabled() {
				j.log.Debug("handled delete", "resource", objKey)
			}
		} else {
			...
			if oldObj != nil && newObj != nil {
				...
				j.log.Error("invalid event: old value is set but new value is nil for non-delete event",
					"input_key", objKey)
				continue
			}
		}
		...
		if j.log.DebugEnabled() {
			j.log.Debug("handled event", "resource", objKey, "event_type", ev.Event)
		}
	...
	if j.log.DebugEnabled() {
		j.log.Debug("calling handlers", "events", len(events))
	}
...
func (j *mergejoin[T]) runQueue() {
	...
	// Sub-collection handlers belong to this join's lifetime: without the
	// deferred unregister the early return below leaves them pushing into a
	// queue that never runs.
	defer func() {
		for _, r := range regs {
			r.UnregisterHandler()
		}
