...
func NewQueue(name string, options ...func(*Queue)) Queue {
	...
	q.logger = log.With("queue", q.name)
	return q
}
...
func (q Queue) AddObject(obj Object) {
	q.queue.Add(namespacedName(obj))
}
...
func (q Queue) ShutDownEarly() {
	q.logger.Info("queue shutdown early")
	q.queue.ShutDown()
}
...
func (q Queue) processNextItem() bool {
	...
	if key == defaultSyncSignal {
		q.logger.Info("queue synced")
		q.initialSync.Store(true)
		return true
	}

	q.logger.Debug("handling update", "key", formatKey(key))
	...
	if err != nil {
		...
		if retryCount < q.maxAttempts {
			q.logger.Warn("error handling update; retrying",
				"key", formatKey(key), "retry_count", retryCount, "error", err)
			q.queue.AddRateLimited(key)
			// Return early, so we do not call Forget(), allowing the rate limiting to backoff
			return true
		}
		q.logger.Error("error handling update; retry budget exceeded", "key", formatKey(key), "error", err)
	}
