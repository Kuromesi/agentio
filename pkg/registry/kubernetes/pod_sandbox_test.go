	...
	for _, kruiseEnabled := range []bool{false, true} {
		t.Run(fmt.Sprintf("kruise=%t", kruiseEnabled), func(t *testing.T) {
			...
			eventually(t, func() bool {
				...
				return s != nil && s.Labels["app"] == "changed" && s.Attester != nil &&
					s.State == model.SandboxStatePending
			}, "Pending Pod retains binding and updates policy labels")
			if err := client.Kube().
				CoreV1().
				Pods("demo").
				Delete(ctx, ambient.Name, metav1.DeleteOptions{}); err != nil {
				t.Fatal(err)
			}
			...
			eventually(t, func() bool {
				return r.Sandboxes.GetKey(oldUID) == nil && r.Sandboxes.GetKey("workload:replacement-uid") != nil &&
					len(r.Sandboxes.List()) == count
		...
		t.Run(fmt.Sprintf("native=%t", native), func(t *testing.T) {
			...
			eventually(t, func() bool {
				for _, name := range []string{"injected", "ambient"} {
					s, _ := getSandbox(name)
					w := getWorkload(name)
					if !slices.Equal(s.PolicyRefs[model.TrafficPolicyType].GetResourceNames(), refs) ||
						len(s.Extensions) != 1 ||
						len(s.GetEgressRouting().GetRoutes()) != 1 ||
						w == nil {
						return false
					}
					if !native &&
						(!slices.Equal(w.AuthorizationPolicies, []string{"demo/selected-egress"}) || len(w.Extensions) != 3) {
						return false
					}
				}
				return true
			...
			for _, name := range []string{"injected", "ambient"} {
				s, _ := getSandbox(name)
				if s.State != sandboxv1.SandboxState_SANDBOX_STATE_PENDING ||
					s.Attester.WorkloadUid != "test//Pod/demo/"+name ||
					s.TrafficPolicy != nil {
			...
			if _, err := client.AgentsAPI().
				...
				Update(ctx, global, metav1.UpdateOptions{}); err != nil {
				t.Fatal(err)
			}
			eventually(t, func() bool {
				updated, ok := snapshot().Get(key)
				return ok && updated.Hash != original.Hash
			}, "shared body update published")
			...
			eventually(t, func() bool {
				...
				return slices.Equal(
					s.PolicyRefs[model.TrafficPolicyType].GetResourceNames(),
					[]string{refs[0], refs[2]},
				) &&
					len(s.Extensions) == 0
			...
			if !native {
				...
				if _, err := client.Kube().
					...
					Update(ctx, config, metav1.UpdateOptions{}); err != nil {
					t.Fatal(err)
				}
				eventually(t, func() bool {
					...
					for _, ext := range w.GetExtensions() {
						if ext.Name == "egress-policies" {
							...
							return ext.Config.UnmarshalTo(p) == nil && len(p.EgressPolicies) == 1 &&
								p.EgressPolicies[0].Policy == extensionsv1.EgressPolicyAction_DENY
						}
					}
					return false
