...
func (ca *CA) Issue(t test.Failer, spec LeafSpec) (tls.Certificate, *x509.Certificate) {
	t.Helper()
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
...
// 127.0.0.1.
func (ca *CA) Loopback(t test.Failer, serial int64, usage x509.ExtKeyUsage) tls.Certificate {
	t.Helper()
...
func SelfSigned(t test.Failer, serial int64) (certPEM, keyPEM []byte) {
	t.Helper()
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
...
// them.
func PEM(t test.Failer, cert tls.Certificate) (certPEM, keyPEM []byte) {
	t.Helper()
...
func encode(t test.Failer, der []byte, key *ecdsa.PrivateKey) (certPEM, keyPEM []byte) {
	t.Helper()
...
func WritePEM(t test.Failer, cert tls.Certificate, certPath, keyPath string) {
	t.Helper()
...
func WriteBundle(t test.Failer, ca *CA, path string) {
	t.Helper()
