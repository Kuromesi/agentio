...
func awaitClientCert(t *testing.T, p certs.Provider, wantCount int) *tls.Certificate {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("presented certificate count = %d, want %d", len(cert.Certificate), wantCount)
		}
...
func TestDynamicServesCAWithoutClientCertificate(t *testing.T) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("anchors from a CA-only source never became available")
		}
		time.Sleep(10 * time.Millisecond)
	}
...
func TestDynamicFallsBackToSystemAnchorsOnSourceError(t *testing.T) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("initial anchors never loaded")
		}
		time.Sleep(10 * time.Millisecond)
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("anchors did not fall back to the system pool after a source error")
		}
		time.Sleep(10 * time.Millisecond)
	}
