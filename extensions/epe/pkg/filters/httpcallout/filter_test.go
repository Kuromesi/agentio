...
func TestDescriptorFailurePolicyIsInverted(t *testing.T) {
	...
	if got := d.OnError(Config{Request: true}); got != filter.FailClosed {
		t.Errorf("OnError(default) = %v, want FailClosed", got)
	}
	if got := d.OnError(Config{Request: true, FailOpen: true}); got != filter.FailOpen {
...
func TestDescriptorSubscribesOnlyWhenTheResponsePhaseIsEnabled(t *testing.T) {
	...
	if got := d.SubscribesOf(Config{Request: true}); got != 0 {
		t.Errorf("SubscribesOf(request only) = %08b, want 0", got)
	}
	if got := d.SubscribesOf(Config{Response: true}); got != filter.PhaseResponseHeaders {
		t.Errorf("SubscribesOf(response) = %08b, want PhaseResponseHeaders", got)
	}
	if got := d.SubscribesOf(Config{Request: true, Response: true}); got != filter.PhaseResponseHeaders {
		t.Errorf("SubscribesOf(both) = %08b, want PhaseResponseHeaders", got)
	}
}

func TestFilterRequestHeadersAsksForTheBodyOnlyWhenEnabled(t *testing.T) {
	ctx := context.Background()
	st := testStream()

	t.Run("request enabled", func(t *testing.T) {
		f := newFilter(t, Config{Request: true}, &fakeClient{})
	...
	t.Run("response only", func(t *testing.T) {
		f := newFilter(t, Config{Response: true}, &fakeClient{})
		act, err := f.OnRequestHeaders(ctx, st)
		if err != nil {
			t.Fatalf("OnRequestHeaders: %v", err)
		}
...
func TestFilterResponseHeadersAsksForTheBodyOnlyWhenEnabled(t *testing.T) {
	ctx := context.Background()
	st := testStream()

	t.Run("response enabled", func(t *testing.T) {
		f := newFilter(t, Config{Response: true}, &fakeClient{})
...
func TestFilterBodyPhasesAreGuardedByConfig(t *testing.T) {
	...
	t.Run("request body without the request phase", func(t *testing.T) {
		client := &fakeClient{}
		f := newFilter(t, Config{Response: true}, client)
	...
	t.Run("response body without the response phase", func(t *testing.T) {
		client := &fakeClient{}
		f := newFilter(t, Config{Request: true}, client)
...
func TestFilterReturnsAnErrorForEveryFailureMode(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			cfg:     Config{Request: true, MaxBodyBytes: 4},
			client:  &fakeClient{},
