...
// yamlDocSeparator matches multi-document YAML separator lines.
var yamlDocSeparator = regexp.MustCompile(`(?m)^---.*$`)

// splitYAML splits multi-document YAML text into its trimmed, non-empty
// documents.
func splitYAML(text string) []string {
	var docs []string
	for _, doc := range yamlDocSeparator.Split(text, -1) {
