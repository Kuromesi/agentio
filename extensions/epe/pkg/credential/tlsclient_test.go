...
func TestNewClientWithCacheDerivesServerNameFromOverriddenURL(t *testing.T) {
	test.SetForTest(t, &insecureSkipVerify, false)
	test.SetForTest(t, &identityProviderURL, "https://from-env.example.com/creds")
...
func TestProviderHostRejectsHostlessURL(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := providerHost(tc.url)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("providerHost(%q) = %q, want an error", tc.url, got)
				}
				return
			}
			if err != nil {
				t.Fatalf("providerHost(%q): %v", tc.url, err)
			}
			if got != tc.want {
				t.Errorf("providerHost(%q) = %q, want %q", tc.url, got, tc.want)
			}
		})
	}
}
