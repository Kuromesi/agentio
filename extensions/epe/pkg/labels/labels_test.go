...
func TestParseSandboxLabels(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: base64.StdEncoding.EncodeToString([]byte("app=sleep,agentio.kruise.io/dataplane-mode=sidecar,app.kubernetes.io/name=sleep")),
			expected: map[string]string{
				"app":                              "sleep",
		...
		{
			name:  "full Kubernetes example",
			input: base64.StdEncoding.EncodeToString([]byte("app=sleep,app.kubernetes.io/name=sleep,app.kubernetes.io/version=v2,pod-template-hash=75d7b8ddbb,agentio.kruise.io/dataplane-mode=sidecar,topology.kubernetes.io/region=cn-hongkong,topology.kubernetes.io/zone=cn-hongkong-c")),
			expected: map[string]string{
				"app":                              "sleep",
				"app.kubernetes.io/name":           "sleep",
