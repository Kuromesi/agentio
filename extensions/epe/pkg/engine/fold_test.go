...
func TestFold(t *testing.T) {
	tests := []struct {
		...
		{
			// Remove-then-Append must keep the remove. Envoy runs all removes
			// before all sets/appends inside one HeaderMutation, so
			// [Remove, Append] is expressible and means "drop the inbound
			// value, then add v". Emitting the append alone makes Envoy append
			...
			// both ops.
			name: "append after remove keeps the remove",
			in: []filter.Mutation{
				filter.RemoveHeader("x-a"),
				filter.AppendHeader("x-a", "v"),
			},
			want: []filter.HeaderOp{
				{Kind: filter.HeaderRemove, Name: "x-a"},
				{Kind: filter.HeaderAppend, Name: "x-a", Value: "v"},
			},
		},
		{
			// Set-then-Remove-then-Append: the set is discarded by the remove,
			// and the remove must still be materialized ahead of the append.
			name: "set remove append keeps the remove",
			in: []filter.Mutation{
		...
		},
		{
			...
			name: "header name case folds across set and append",
			in: []filter.Mutation{
		...
		},
		{
			...
			name: "append preserves ordered multi-value",
			in: []filter.Mutation{
				filter.AppendHeader("set-cookie", "a=1"),
				filter.AppendHeader("set-cookie", "b=2"),
			},
			want: []filter.HeaderOp{
				{Kind: filter.HeaderAppend, Name: "set-cookie", Value: "a=1"},
				{Kind: filter.HeaderAppend, Name: "set-cookie", Value: "b=2"},
			},
		},
		{
			name: "set overrides earlier appends",
			in: []filter.Mutation{
				filter.AppendHeader("x-a", "1"),
