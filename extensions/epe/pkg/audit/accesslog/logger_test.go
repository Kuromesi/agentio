...
// waitFor polls fn until it returns true or timeout fires. Used to give the
// worker goroutine time to drain the channel without resorting to ad-hoc
// sleeps in every test.
func waitFor(t *testing.T, timeout time.Duration, fn func() bool) {
	t.Helper()
	testsupport.Eventually(t, timeout, func() error {
		if fn() {
			return nil
		}
...
// TestBufferedLogger_RejectedAdmissionIncrementsAggregateCounter verifies
// that the existing scalar counter aggregates rejected admissions across
// queue lifecycle reasons rather than exposing a new labelled metric.
func TestBufferedLogger_RejectedAdmissionIncrementsAggregateCounter(t *testing.T) {
	logger, _ := newRecorderLogger()
	const cap = 4
	...
	if err := l.d.Drain(drainCtx); err != nil {
		cancelDrain()
		cancel()
		t.Fatalf("drain accepted entries: %v", err)
	}
	cancelDrain()
	cancel()
	select {
	...
	// Admission after shutdown is rejected with a different queue reason,
	// but accesslog intentionally aggregates every reason in one counter.
	l.Submit(sampleEntry("stopped"))
	const wantRejected = overflow + 1
	if got := counterValue(t, AuditLogDroppedTotal); got-before != wantRejected {
		t.Errorf("expected aggregate rejected-admission delta %d, got %v (before=%v)", wantRejected, got-before, before)
	}
}
