...
Pull requests and master pushes use `agentio-e2e-presubmit.yml`. It builds agentiod, EPE, ztunnel from the source SHA in `agentio.deps`, and the ext-proc test fixture once, transfers them to each matrix job as a same-run artifact, and publishes them only to that job's KinD-local registry. CNI, proxy-init, and gateway remain the immutable public images recorded in `agentio.deps`, so presubmit needs no registry credentials. Release calls the same product workflow with an immutable candidate BOM and promotes only the digests that passed it. See [Building and updating ztunnel](../../docs/development/ztunnel-build.md) for the source sync and publication flow.
