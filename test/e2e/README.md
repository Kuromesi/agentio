...
- `suites/trafficpolicy`: Workload TrafficPolicy matrix and an explicit native-binding check without synthetic Sandboxes and the control-plane config debug surface. The fixture contains `client`, `server`, `another-server`, and two `workload-target` Pods using the selected dataplane path. The matrix covers basic ingress and egress rules, global policy and priority, selector expressions, workload and service peers, TCP/UDP/ICMP protocol rules, policy interaction, Cartesian port/source cases, and selectorless Services with manual EndpointSlices. Policy documents use explicit `to`/`from` peers, including port-only and protocol-only restrictions.
...
- `suites/epe`: the Egress Policy Enforcer attribute, RBAC, metrics, profile-priority, and ordinary-Workload Secret token injection contracts against Envoy. Agentgateway EPE coverage is deferred until production peer attributes are available.
...
Policy checks read workload-scoped JSON from both sidecar and ambient proxies. Native checks require a Workload TrafficPolicy reference and the corresponding cached policy body; an unrelated cached policy does not count as applied. Legacy checks identify each source policy by its ingress/egress Authorization names and verify its Workload reference or applicable global/namespace scope. Real-traffic convergence assertions then verify enforcement. Egress routing tests use `GATEWAY`/`PASSTHROUGH`, with separate TrafficPolicy tests for rejection.

All supplied suite images must be immutable digest references. The forward proxy fixture uses a framework-owned Envoy digest by default; set `AGENTIO_E2E_FORWARD_PROXY_IMAGE` only to override it. Release and manual E2E workflows build the repository's ext-proc fixture into each job's local registry when no `ext_proc_image` override or candidate archive is supplied. CI generates four isolated environment groups from the same planner used locally. The two `auto` groups run the suites that require their profile; the two `iptables` groups run only trafficpolicy. This schedules 11 suite executions instead of 18. clienttrust runs only in `sidecar-auto`. Suite setup verifies the requested `FIREWALL_BACKEND` on the injected sidecar or ambient ztunnel Pod before any product assertion runs.

Pull requests and master pushes use `agentio-e2e-presubmit.yml`. It builds agentiod, EPE, ztunnel from the source SHA in `agentio.deps`, and the ext-proc test fixture once, transfers them to each matrix job as a same-run artifact, and publishes them only to that job's KinD-local registry. Presubmit caches the ztunnel binary by source repository, commit, architecture, runner OS, and build workflow; cache hits skip compilation but still verify the architecture and smoke-test the runtime image. Published release candidates always compile from source. CNI, proxy-init, and gateway remain the immutable public images recorded in `agentio.deps`, so presubmit needs no registry credentials. Release calls the same product workflow with an immutable candidate BOM and promotes only the digests that passed it.
...
This matrix covers native Workload policies on ordinary Pods without synthetic Sandboxes. It does not claim coverage of a real Sandbox UID and structured Workload-to-Sandbox binding; that remains a separate future scenario.
