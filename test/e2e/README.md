...
Policy checks read workload-scoped JSON from both sidecar and ambient proxies. Native checks require a Sandbox reference and the corresponding cached TrafficPolicy body; an unrelated cached policy does not count as applied. Legacy compatibility Authorizations aggregate several source policies, so the dump cannot prove that an individual source policy was added or removed. Tests report that limitation explicitly and use the following real-traffic convergence assertions; allow-only probes do not independently prove source-policy propagation. Egress routing tests use `GATEWAY`/`PASSTHROUGH`, with separate TrafficPolicy tests for rejection.
