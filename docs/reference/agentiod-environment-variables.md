...
`AGENTIO_NATIVE_SANDBOX_POLICIES=false` (the default) also emits legacy Workload policies. Each shared TrafficPolicy produces its own ingress/egress Authorization resources, retaining the source name, priority and global/namespace/selector scope. Global and namespace policies apply through the legacy data plane's scope handling; selector policies are referenced by the Workload bound to the selected Sandbox. Sandbox-owned TrafficPolicy rules produce separate workload-scoped Authorizations with priority `-1`, ahead of shared policies. The compatibility output adds no terminal allow/deny rules; unmatched policies, missing resources and equal priorities retain the legacy data plane's behavior. SNI and egress policies use the existing Workload extensions. Setting `AGENTIO_NATIVE_SANDBOX_POLICIES=true` disables this compatibility output for both derived Pod Sandboxes and runtime Sandboxes. Native policy selection uses Sandbox namespace, labels and explicit references; legacy namespace Authorizations use the backing Workload's namespace. Workloads without a bound Sandbox do not independently select selector policies, but applicable legacy global and namespace policies still apply.

Compatibility requires one Sandbox per Workload. Ambiguous bindings are reported as compilation errors and do not produce a Sandbox-to-Workload policy projection. Shared Authorization resources are compiled once per source policy and reused across Workloads and ADS connections.
