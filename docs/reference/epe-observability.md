...
While the port-forward is running, use `grpc-health-probe` from another
terminal:
...
EPE does not enable the gRPC reflection API. A bare `grpcurl` invocation
without a health-service proto or protoset therefore cannot resolve
`grpc.health.v1.Health`, even when the health listener is serving normally.
...
| `epe_profile_compile_failures_total` | Counter; `scope` (`namespaced`, `global`, or `inline`) | Published policy versions that EPE rejected during compilation. `inline` counts per-Sandbox annotation rule chains. |
...
| `epe_profile_unenforced` | Gauge; `scope` | How many policy sources in that scope have no installed version at all, so none of their rules are enforced. For `inline`, that Sandbox's own rules are absent while administrator profiles still apply. |
