...
| `/var/run/secrets/istio`, `/var/run/secrets/istio/crl`, `/var/lib/istio/data`, `/etc/istio/proxy`, `/etc/istio/pod`, projected `istio-token` file | Gateway and ztunnel templates | Paths read by the existing proxy/ztunnel binaries. Certificate, token, bootstrap, pod metadata and CRL paths must migrate with those binaries. |
...
stats annotations. Old CNI images do not understand the new traffic/status
