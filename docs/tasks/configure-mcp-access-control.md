...
Because this task uses `https://mcp.example.com`, configure the selected gateway to terminate TLS for `mcp.example.com`, configure the MITM signing CA, and make this workload trust that CA. Otherwise the connection remains on the gateway's TCP path and bypasses EPE's HTTP filter chain. Follow [TLS termination for HTTPS inspection](../reference/agentio-configuration.md#tls-termination-for-https-inspection).
