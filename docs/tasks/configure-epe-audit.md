...
The example below matches `https://api.example.com`. Configure the selected gateway to terminate TLS for `api.example.com`, configure the MITM signing CA, and make the workload trust that CA before testing it. Without those settings, HTTPS remains on the gateway's TCP path and bypasses EPE's HTTP filter chain. Follow [TLS termination for HTTPS inspection](../reference/agentio-configuration.md#tls-termination-for-https-inspection).
...
EPE does not retry audit webhooks. It records a `success`, `http_error`, `transport_error`, or `timeout` in `epe_audit_webhook_dispatched_total`; HTTP responses with status 400 or higher are `http_error`. Rendering failures, non-HTTP(S) URLs, and bodies larger than 64 KiB are dropped before dispatch and reported through `epe_audit_webhook_dropped_total`. EPE verifies HTTPS audit webhook certificates by default. Keep `epe.auditWebhook.insecureSkipVerify=false`; setting it to `true` disables server identity verification and is unsafe.

The Helm chart exposes audit-webhook TLS verification as `epe.auditWebhook.insecureSkipVerify`. Audit queue, worker, and response-observation flags still require an explicit container argument override or a derived deployment/image; `epe.env` cannot set command-line flags. See [EPE configuration](../reference/epe-configuration.md).
