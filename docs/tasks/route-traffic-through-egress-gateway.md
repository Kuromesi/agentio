...
You can create the gateway with either the Kubernetes Gateway API or the Agentio Helm chart. Both methods support sidecar, ambient, and OpenKruise Agents onboarding paths. Choose one method, then configure the common routing policy.
...
Use this method when the cluster has the Gateway API CRDs and Agentio manages the `istio-waypoint` GatewayClass.

Verify the required API and GatewayClass:
...
Create an Istio waypoint and label it as an Agentio sandbox egress gateway:
...
Agentio copies the Gateway labels to the managed waypoint resources. The `networking.agents.kruise.io/sandbox-egress: "true"` label makes the control plane treat this waypoint as an egress gateway.

Wait for the Gateway and its managed workload:

```console
$ kubectl wait --for=condition=Programmed \
    gateway/agentio-egress \
    --namespace agentio-system \
    --timeout=2m
...
Continue with [Configure egress routing](#configure-egress-routing).

## Create the gateway with Helm

Use this method when you want the Agentio chart to manage the gateway Deployment and supporting resources directly.

Create an evaluation values file:

```console
$ cat >/tmp/agentio-egress-gateway-values.yaml <<'EOF'
egressGateway:
  autoscaling:
    enabled: false
  replicas: 1
...
$ helm upgrade agentio ./manifests/charts/agentio \
    --namespace agentio-system \
    --reuse-values \
    --set-json 'egressPolicies=[]' \
    --wait \
    --timeout 5m
```

If you created the waypoint with the Gateway API, remove it with:

```console
$ kubectl delete gateway agentio-egress --namespace agentio-system
```

If you created the gateway with Helm, remove the chart-managed gateway entry:

```console
$ helm upgrade agentio ./manifests/charts/agentio \
    --namespace agentio-system \
    --reuse-values \
    --set-json 'egressGateway.gateways=[]' \
    --wait \
    --timeout 5m
```
