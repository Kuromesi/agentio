...
func TestVerifyIntegrationBundleRejectsVersionMetadataInTemplates(t *testing.T) {
	...
	template := "sandbox-manager/templates/agentio/agentiod/deployment.yaml"
...
func TestVerifyIntegrationBundleRejectsVersionMetadataInFiles(t *testing.T) {
	...
	file := "sandbox-manager/files/agentio/crds/trafficpolicies.yaml"
...
{{- end }}
`
	writeTestFile(t, bundle, "templates/agentio/agentiod/deployment.yaml", preparedTemplate)
	...
	assertTestFile(t, target, "templates/agentio/agentiod/deployment.yaml", preparedTemplate)
...
func TestRepositoryAgentioChartCanBeExported(t *testing.T) {
	...
	if !strings.Contains(readTestFile(t, target, "templates/agentio/agentiod/deployment.yaml"), ".Values.agentio.agentiod") {
...
func TestRepositoryOpenKruiseIntegrationBundleIsCurrent(t *testing.T) {
	...
	if got != want {
		t.Fatal("checked-in OpenKruise integration bundle is stale; run go run ./tools/agentio-chart-sync build --chart manifests/charts/agentio")
	}
}
...
func TestExportOmitsAmbientAndSidecarInjectionFromSandboxManager(t *testing.T) {
	...
	for name := range sandboxManagerExcludedTemplates {
	...
	for _, key := range []string{"profile", "cni", "ztunnel"} {
	...
	agentiod := readTestFile(t, target, "templates/agentio/agentiod/deployment.yaml")
	...
	for _, unwanted := range []string{"AGENTIO_INJECTION_WEBHOOK_CONFIG_NAME", "https-webhook", "15017"} {
		if strings.Contains(agentiod, unwanted) {
			t.Errorf("generated agentiod template retained sidecar injector setting %q", unwanted)
		}
	}
	if !strings.Contains(agentiod, "name: AGENTIO_ENABLE_SIDECAR_INJECTOR\n              value: {{ false | quote }}") {
		t.Fatalf("generated agentiod template does not disable its injector:\n%s", agentiod)
	}
	clusterRole := readTestFile(t, target, "templates/agentio/agentiod/rbac.yaml")
...
func TestRepositoryAgentioCRDsAreAlwaysRenderedAndRetained(t *testing.T) {
	...
	rendered := renderChart(t, target, nil)
...
func renderAgentioResourceNamespaces(t *testing.T, target string, userValues map[string]any) map[string]string {
	t.Helper()
	rendered := renderChart(t, target, userValues)
...
func renderChart(t *testing.T, target string, values map[string]any) map[string]string {
	t.Helper()
	...
	if err := os.WriteFile(valueFile, encoded, 0o644); err != nil {
		t.Fatal(err)
	}
	output, err := exec.Command("helm", "template", "test", target, "--namespace", "sandbox-system", "-f", valueFile).CombinedOutput()
	if err != nil {
	...
	for _, doc := range strings.Split(string(output), "\n---") {
		for _, line := range strings.Split(doc, "\n") {
			if name, ok := strings.CutPrefix(line, "# Source: "); ok {
				rendered[name] += doc + "\n---\n"
				break
			}
		}
