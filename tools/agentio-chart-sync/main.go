...
func run(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: agentio-chart-sync <build|apply|verify> [flags]")
		return 2
	}
	switch args[0] {
	case "build":
...
func runBuild(args []string) error {
	...
	output := flags.String("output", "", "bundle directory (default: <chart>/integrations/openkruise)")
	if err := flags.Parse(args); err != nil {
		return err
	}
	if flags.NArg() != 0 || *chart == "" {
		return errors.New("--chart is required")
	}
	if *output == "" {
		*output = filepath.Join(*chart, "integrations", "openkruise")
	}
	return BuildIntegrationBundle(*chart, *output)
}
