...
func TestSniPolicyFeatureFlagOff(t *testing.T) {
	...
	for _, sub := range []struct {
		...
		assert.Equal(t, len(removed), 0)
		assert.Equal(t, usedDelta, true)
		// Both must be non-nil, or pushDeltaXds drops the response entirely and the
		// client waits forever for a first response it uses to decide readiness.
		if res == nil || removed == nil {
			t.Fatalf("flag-off delta must return empty non-nil slices, got res=%#v removed=%#v", res, removed)
		}
	}
	assert.Equal(t, stub.calls[v3.PolicyBindingType], 0)
	assert.Equal(t, stub.calls[v3.SniTrafficPolicyType], 0)
